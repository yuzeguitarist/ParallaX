...
impl Controller for NewReno {
    fn on_ack(&mut self, bytes: u64, app_limited: bool) {
        ...
        if self.window < self.ssthresh {
            ...
            // Congestion avoidance: ~1 MSS per RTT (RFC 9002 §7.3.2).
            self.window += MAX_DATAGRAM_SIZE * bytes / self.window;
        }
    }
...
mod tests {
    ...
    fn congestion_event_halves_then_avoids() {
        ...
        // Now in congestion avoidance (window == ssthresh): +mss^2/cwnd per ack.
        cc.on_ack(MAX_DATAGRAM_SIZE, false);
        assert_eq!(cc.window(), 6_000 + (1_200 * 1_200 / 6_000));
    }
