...
- **Non-goal:** there is no shipped `--quic` product transport on current
  `main`; QUIC appears only in research/simulator context.
...
| Minimum Rust | `1.80` | [`../Cargo.toml`](../Cargo.toml) |
| CLI commands | `check`, `keygen`, `crypto-self-test`, `serve`, `client`, `speed`, `bench`, `config-template`, `probe`, `init` | [`../src/cli.rs`](../src/cli.rs), `plx --help` |
| Default product transport | TCP with TLS-shaped camouflage records | [`../src/transport/tcp.rs`](../src/transport/tcp.rs), [`Transport Layer`](Transport-Layer.md) |
...
| Data plane | XChaCha20-Poly1305 records inside TLS `ApplicationData` | [`../src/protocol/data.rs`](../src/protocol/data.rs), [`Session Key Derivation & AEAD Transport`](Session-Key-Derivation-&-AEAD-Transport.md) |
...
| Encrypted records and relay payloads | `PX1C`, `PX1Q`, `PX1K`, `PX1S`, `ApplicationData`, `XChaCha20-Poly1305` | [`Protocol Commands & Data Records`](Protocol-Commands-&-Data-Records.md) |
| Performance or throughput evidence | `plx speed`, `plx bench`, `network evidence`, `benchmark`, `53 cases` | [`Probing & Benchmarking`](<Probing-&-Benchmarking.md>), [`Protocol Benchmarks`](Protocol-Benchmarks.md) |
...
| `doc-id:crypto.session` | [`Session Key Derivation & AEAD Transport`](Session-Key-Derivation-&-AEAD-Transport.md) | implementer, reviewer | HKDF chain secrets, directional keys, AEAD codec. | [`../src/crypto/session.rs`](../src/crypto/session.rs), [`../src/protocol/data.rs`](../src/protocol/data.rs) | [`Protocol Commands & Data Records`](Protocol-Commands-&-Data-Records.md), [`Post-Quantum Cryptography (ML-KEM & ML-DSA)`](<Post-Quantum-Cryptography-(ML-KEM-&-ML-DSA).md>) | HKDF, chain secret, XChaCha20-Poly1305, nonce |
...
| `doc-id:transport.index` | [`Transport Layer`](Transport-Layer.md) | implementer, operator | Product transport boundaries and non-QUIC stance. | [`../src/transport/`](../src/transport/) | [`TCP Camouflage Transport`](TCP-Camouflage-Transport.md), [`GFW Simulator & QUIC Research`](<GFW-Simulator-&-QUIC-Research.md>) | TCP, transport, no UDP, no --quic |
...
- Root deployment material should link to [`Deployment`](Deployment.md) or the
  ...
  docs.
- The `--quic` spelling may appear only as a negative statement that the product CLI has no
  such option.
